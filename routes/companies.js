...
router.post('/', async (req, res) => {
  try {
    const { nome, cnpj, tipo, vencimento, email_empresa, email_interno, obs } = req.body;
    if (!nome || !vencimento)
      return res.status(400).json({ error: 'Nome e vencimento são obrigatórios.' });
    const row = await db.getAsync(
      `INSERT INTO companies (user_id,nome,cnpj,tipo,vencimento,email_empresa,email_interno,obs)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *`,
      req.user.id, nome.trim(), cnpj||null, tipo||null, vencimento,
      email_empresa||null, email_interno||null, obs||null
    );
...
router.put('/:id', async (req, res) => {
  try {
    ...
    const updated = await db.getAsync(
      `UPDATE companies
       SET nome=$1,cnpj=$2,tipo=$3,vencimento=$4,email_empresa=$5,email_interno=$6,obs=$7,
           updated_at=NOW()
       WHERE id=$8 RETURNING *`,
      nome||row.nome, cnpj||null, tipo||null, vencimento||row.vencimento,
      email_empresa||null, email_interno||null, obs||null, row.id
    );
    res.json(updated);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
